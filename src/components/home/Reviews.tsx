...
export function Reviews({ title, desc, items }: ReviewsProps = {}) {
  const { t } = useLanguage();

  const displayTitle = title || t("reviews.title");
  const displayDesc = desc || t("reviews.desc");
  const displayItems = items || DEFAULT_REVIEWS.map(r => ({
    id: r.id,
    name: r.name,
    avatar: r.avatar,
    text: t(`reviews.r${r.id}_text`),
    role: t(`reviews.r${r.id}_role`)
  }));
    ...
    <section id="reviews" className="py-16 sm:py-24 bg-[var(--background)]">
      <div className="container mx-auto px-4">
        <div className="text-center mb-10 sm:mb-16">
          <h2 className="text-3xl sm:text-4xl font-bold mb-4 text-[var(--foreground)]">{displayTitle}</h2>
          <p className="text-[var(--muted-text)] text-base sm:text-lg max-w-2xl mx-auto font-medium">{displayDesc}</p>
        </div>
          ...
          {displayItems.map((review) => (
            <div key={review.id} className="flex flex-col glass-card rounded-2xl p-6 sm:p-8 border border-[var(--card-border)] hover:border-indigo-500/30 hover:bg-white/5 transition-all duration-300 h-full w-[80vw] sm:w-auto shrink-0 md:shrink">
              ...
              <div className="flex-1">
                 <p className="text-[var(--foreground)] text-base leading-relaxed mb-8 font-medium opacity-90 italic">"{review.text}"</p>
              </div>
              <div className="flex items-center gap-4 mt-auto pt-6 border-t border-[var(--card-border)]">
                ...
                <div>
                  ...
                  <div className="text-xs text-[var(--muted-text)] font-bold uppercase tracking-wider">{review.role}</div>
                </div>
              </div>
            </div>
          ))}
