...
def main():
    print("=" * 60)
    print("STARTING DYNAMIC BLOG METADATA TRANSLATION & INJECTION")
    print("=" * 60)
    ...
    if "blog" not in en_data:
        print("[ERROR] 'blog' section not found in en.json.")
        sys.exit(1)
        
    blog_en = en_data["blog"]
    flat_en_items = flatten_dict(blog_en)
    print(f"Loaded English structures for Blog. Total flattened strings to translate: {len(flat_en_items)}")
    ...
    for idx, filename in enumerate(target_files):
        ...
        translated_blog = unflatten_dict(translated_flat)
        
        # Inject back into the locale under 'blog'
        target_data["blog"] = translated_blog
    ...
    print("\n" + "=" * 60)
    print("DYNAMIC BLOG TRANSLATION & INJECTION COMPLETE!")
    print("=" * 60)
