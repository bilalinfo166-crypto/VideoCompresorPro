...
SECTIONS = ["tiktok_page", "reels_page", "shorts_page"]
...
def main():
    print("=" * 60)
    print("STARTING PLATFORMS LANDING PAGES TRANSLATION & INJECTION")
    print("=" * 60)
    ...
    # We will translate all 3 platform sections
    platform_en = {}
    for sec in SECTIONS:
        if sec in en_data:
            platform_en[sec] = en_data[sec]
            
    if not platform_en:
        print("[ERROR] No platform sections found in en.json.")
        sys.exit(1)
        
    flat_en_items = flatten_dict(platform_en)
    print(f"Loaded English structures for Platform pages. Total flattened strings to translate: {len(flat_en_items)}")
    ...
    for idx, filename in enumerate(target_files):
        ...
        translated_platforms = unflatten_dict(translated_flat)
        
        # Inject back into the locale
        for sec in SECTIONS:
            if sec in translated_platforms:
                target_data[sec] = translated_platforms[sec]
    ...
    print("\n" + "=" * 60)
    print("PLATFORMS LANDING PAGES TRANSLATION & INJECTION COMPLETE!")
    print("=" * 60)
