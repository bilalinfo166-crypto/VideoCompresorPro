...
def main():
    print("=" * 60)
    print("STARTING NEW PSEO PAGES TRANSLATION & INJECTION")
    print("=" * 60)
    ...
    # Extract only the 7 new keys from en.json
    pseo_en = en_data["pseo"]
    new_pages = {}
    for key in NEW_PSEO_KEYS:
        if key not in pseo_en:
            print(f"[ERROR] New key '{key}' not found under 'pseo' in en.json.")
            sys.exit(1)
        new_pages[key] = pseo_en[key]
        
    # Flatten the new pages data structure
    flat_en_items = flatten_dict(new_pages)
    print(f"Loaded English structures for new pages. Total flattened strings to translate: {len(flat_en_items)}")
    ...
    for idx, filename in enumerate(target_files):
        ...
        for key in NEW_PSEO_KEYS:
            target_data["pseo"][key] = translated_new_pages[key]
    ...
    print("\n" + "=" * 60)
    print("NEW PSEO PAGES TRANSLATION & INJECTION COMPLETE!")
    print("=" * 60)
