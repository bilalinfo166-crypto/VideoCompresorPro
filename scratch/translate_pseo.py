...
def flatten_dict(d, parent_key=''):
    """
    Flattens a dictionary/list structure into a flat key path representation.
    """
    items = []
...
def unflatten_dict(flat_items):
    """
    Reconstructs a nested dict/list structure from its flattened representation.
    """
    res = {}
    for flat_key, val in flat_items.items():
        parts = flat_key.split('.')
        curr = res
        for idx, part in enumerate(parts):
            # Convert digit string to int if it's for list indexing
            if part.isdigit():
                part = int(part)
...
def main():
    print("=" * 60)
    print("STARTING PSEO YOUTUBE & TIKTOK TRANSLATION INJECTION")
    print("=" * 60)
    ...
    if "pseo" not in en_data:
        print("[ERROR] 'pseo' section not found in en.json.")
        sys.exit(1)
        
    # Get the master YouTube and TikTok blocks
    pseo_en = en_data["pseo"]
    if "youtube" not in pseo_en or "tiktok" not in pseo_en:
        print("[ERROR] 'youtube' or 'tiktok' not found under 'pseo' inside en.json.")
        sys.exit(1)
        
    new_pages = {
        "youtube": pseo_en["youtube"],
        "tiktok": pseo_en["tiktok"]
    }
    
    # Flatten the new pages data structure
    flat_en_items = flatten_dict(new_pages)
    print(f"Loaded English structures. Total flattened strings to translate: {len(flat_en_items)}")
    ...
    for idx, filename in enumerate(target_files):
        ...
        # Run flat translation
        t0 = time.time()
        translated_flat = batch_translate(flat_en_items, google_lang)
        duration = time.time() - t0
        
        print(f"  Translated all strings in {duration:.1f}s.")
        
        # Unflatten to restore nested dictionary structure
        translated_new_pages = unflatten_dict(translated_flat)
        
        # Inject back into the locale under 'pseo'
        if "pseo" not in target_data:
            target_data["pseo"] = {}
            
        target_data["pseo"]["youtube"] = translated_new_pages["youtube"]
        target_data["pseo"]["tiktok"] = translated_new_pages["tiktok"]
        
        # Save the updated target file with 2 spaces indentation
        try:
    ...
    print("\n" + "=" * 60)
    print("YOUTUBE & TIKTOK PSEO TRANSLATION & INJECTION COMPLETE!")
    print("=" * 60)
