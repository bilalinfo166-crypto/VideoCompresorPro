...
def batch_translate(items, target_lang):
    ...
    # Process in chunks of 10 items for speed and safety
    chunk_size = 10
...
def main():
    print("=" * 60)
    print("STARTING 30-LANGUAGE VIDEO TO TEXT TRANSLATION INJECTION")
    print("=" * 60)
    ...
    if "to_text_page" not in en_data:
        print("[ERROR] 'to_text_page' section not found in en.json.")
        sys.exit(1)
        
    en_to_text_section = en_data["to_text_page"]
    en_items = list(en_to_text_section.items())
    print(f"Loaded Master English 'to_text_page' with {len(en_items)} keys.")
    
    # Scan locales directory
    all_files = sorted([f for f in os.listdir(LOCALES_DIR) if f.endswith('.json') and f != 'en.json'])
    print(f"Found {len(all_files)} target locale files to translate.")
    ...
    for idx, filename in enumerate(all_files):
        ...
        print(f"\n[{idx+1}/{len(all_files)}] Processing '{filename}' (target: '{google_lang}')...")
        ...
        target_data["to_text_page"] = translated_section
